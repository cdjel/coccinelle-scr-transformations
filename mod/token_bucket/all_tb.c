 ...
 void fast_forward_state(void *data, int index, struct flow_key *key)
 {
   for(int j = 0;j < NUM_META;j++) {
     ...
     if (meta->l3proto != htons(ETH_P_IP) || meta->l4proto != IPPROTO_TCP)
       continue;
     struct elem *value = bpf_map_lookup_elem(&token_map, &key);
     if (!value) {
       struct elem init_state = {
         .num = MAX_TOKEN,
         .last_time = bpf_ktime_get_ns()
         };
       bpf_map_update_elem(&token_map, &key, &init_state, BPF_ANY);
       continue;
     }
     ...
     bpf_map_update_elem(&token_map, &key, value, BPF_ANY);
   }
 }
 struct metadata {
   int l3proto;
