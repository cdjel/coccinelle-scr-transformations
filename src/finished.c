#define KBUILD_MODNAME "scr_portknock"
...
int xdp_prog(struct xdp_md *ctx) {
    ...
    if ((void *)pkt + sizeof(*pkt) > data_end)
        return XDP_DROP;

    if (pkt->eth.h_proto != htons(ETH_P_IP) || pkt->ip4.protocol != IPPROTO_TCP)
        return XDP_DROP;
